...
3. **服务端整理索引**：按时间倒序扫描 `.journal/memory/yyMM/*.md`（最多 120 条），再扫描 `.journal/identity/*.md`（本人 → 产品 → 其他人，按团队分组）；每条只取 frontmatter 里的 `summary` / `tags` / `sources` 和标题。每个文件带指纹缓存（fs 服务的 `version` 令牌或 mtimeMs+size）：没变的文件直接复用上次的解析结果，重开面板不再全量重读；文件一变即自动重读（见 ADR-0014）。
4. **浏览器渲染**：所有内容先做 HTML 转义再放进页面（防止日志内容破坏页面），卡片上绑定点击事件。列表先渲染前几个分组、滚动到底自动追加（渐进渲染）；搜索框在浏览器侧直接按标题/摘要/标签过滤（数据已在内存，不加服务端搜索接口，见 ADR-0015）。
...
- 保存走**读-改-写**：以磁盘上的现配置文件为基底、只覆盖本次提交的字段——两个 DSH 会话并行保存也不会互相覆盖（见 ADR-0014）；
