...
const DEFAULTS = {
  ...
  startupContext: true, // 是否注入启动上下文（摘要 + 书写规范）
  writeProtocolEnabled: true, // 是否注入书写规范（摘要不受影响）
  writeProtocol: '', // 自定义书写规范；空 = 用内置默认（WRITE_PROTOCOL）
}
...
const CONFIG_RULES = {
  ...
  writeProtocolEnabled: { kind: 'bool' },
  writeProtocol: { kind: 'string', max: 50000 },
}
...
/** 读取 <root>/.jinji-memory.json 并并入 runtimeConfig；文件不存在/损坏时保持现状。 */
async function loadConfigFile(fs, root, runtimeConfig) {
  try {
    const target = await fs.resolve(root + '/' + CONFIG_FILE)
    const st = await fs.stat(target)
    if (st === undefined || st.type !== 'file') return false
    mergeConfig(runtimeConfig, JSON.parse(await fs.readText(target)))
    return true
  } catch {
    return false
  }
...
/** 把当前 runtimeConfig 全量写回配置文件（原子写，走 fs 服务）。 */
async function saveConfigFile(fs, root, runtimeConfig) {
  const target = await fs.resolve(root + '/' + CONFIG_FILE)
  const body = {}
  for (const field of Object.keys(CONFIG_RULES)) body[field] = runtimeConfig[field]
...
export function apply(ctx, config = {}) {
  ...
  loadConfigFile(ctx.fs, root, runtimeConfig)

  // ── 启动注入（摘要见 ADR-0009，书写规范见 ADR-0010） ─────────────────
  ...
  ctx.inject(['systemPrompt'], (scope) => {
    ...
    scope.systemPrompt.context({
      ...
      text: (assembleCtx) => {
        const agent = assembleCtx && assembleCtx.agent
        if (agent === undefined || !runtimeConfig.startupContext || !runtimeConfig.writeProtocolEnabled) return ''
        const entry = summaryCache.get(agent)
        const memRoot = entry && entry.root ? entry.root : '（会话工作目录）'
        const custom = runtimeConfig.writeProtocol.trim()
        return (custom || WRITE_PROTOCOL).replace('__MEMORY_ROOT__', memRoot)
      },
    })
  })
        ...
        if (action === 'config') {
          if (req.method === 'GET') {
            return sendJson(res, 200, {
              ...
              defaults: { ...DEFAULTS },
              protocolBuiltin: WRITE_PROTOCOL,
              file: CONFIG_FILE,
            })
          }
          if (req.method === 'POST') {
            ...
            await saveConfigFile(ctx.fs, root, runtimeConfig)
