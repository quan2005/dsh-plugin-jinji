...
async function installSecretaryPreset(ctx, root) {
  ...
  const compTarget = await ctx.fs.resolve(preset.path)
  const comp = await ctx.fs.readText(compTarget)
  await ctx.fs.writeText(compTarget, rewritePersona(comp, buildSecretaryPersona(root)))
  const metaTarget = await ctx.fs.resolve(preset.path.replace(/[^/]+$/, 'preset.yml'))
  await ctx.fs.writeText(metaTarget, SECRETARY_PRESET_META)
