/**
 * dsh-plugin-jinji — Host 半：谨迹记忆面板的数据路由。
 *
 * 零依赖双半插件（见学习手册第 8 章模式）：
 * - 通过 `fs` 服务读取日志库（.journal/memory 最近日志 + .journal/identity 画像）
 * - 通过 `webServer` 注册 GET /api/jinji-memory 路由，为浏览器半提供 JSON
 * - 路径读取有防护：只允许 .journal/ 下的文件，拒绝 `..` 与越界
 * - 日志库根目录：config.root > DSH_JINJI_ROOT 环境变量 > dsh 进程工作目录
 */
