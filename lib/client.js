...
.jm-overlay { position: fixed; top: 0; right: 0; bottom: 0; left: 260px; z-index: 400; background: #0D0F12; color: #C9CFD6; font-family: -apple-system, BlinkMacSystemFont, 'PingFang SC', 'Segoe UI', 'Helvetica Neue', Arial, sans-serif; font-size: 13px; display: flex; flex-direction: column; pointer-events: auto; border-left: 1px solid #23282E; }
...
.jm-topbar { height: 46px; flex-shrink: 0; display: flex; align-items: center; justify-content: center; border-bottom: 1px solid #23282E; background: #0D0F12; position: relative; }
...
.jm-count { position: absolute; right: 20px; font-size: 11.5px; color: #7A828C; }
.jm-close { position: absolute; left: 20px; font-size: 11.5px; color: #7A828C; background: none; border: 1px solid #23282E; border-radius: 6px; padding: 3px 10px; cursor: pointer; }
.jm-close:hover { color: #E4E8ED; border-color: #31373F; }
.jm-view { flex: 1; min-height: 0; display: flex; flex-direction: column; background: #14171B; }
...
.jm-preview { flex: 1; min-width: 0; overflow-y: auto; padding-bottom: 60px; background: #14171B; }
.jm-phead { height: 42px; display: flex; align-items: center; gap: 10px; padding: 0 20px; border-bottom: 1px solid #23282E; position: sticky; top: 0; background: #14171B; z-index: 5; }
.jm-back { display: flex; align-items: center; gap: 5px; color: #7A828C; font-size: 12px; cursor: pointer; padding: 4px 11px; border-radius: 6px; border: 1px solid #23282E; background: none; font-family: inherit; }
.jm-back:hover { color: #E4E8ED; border-color: #31373F; }
.jm-ptitle { font-size: 13px; font-weight: 600; color: #E4E8ED; }
.jm-right { margin-left: auto; }
...
.jm-list { width: 330px; flex-shrink: 0; border-left: 1px solid #23282E; background: #14171B; overflow-y: auto; padding: 10px 10px 40px; }
...
`
    ...
    // 「设置 → 插件配置」里的配置卡片样式（跟随官方主题变量，变量缺失时用同色系兜底）
    const CFG_CSS = `
.jm-cfg-card { border: 1px solid var(--dsw-alias-border-l2, #31373F); border-radius: 10px; padding: 14px 16px; background: var(--dsw-alias-bg-l1, #14171B); display: flex; flex-direction: column; gap: 12px; color: var(--dsw-alias-label-primary, #E4E8ED); font-size: 13px; }
.jm-cfg-title { font-size: 14px; font-weight: 600; }
.jm-cfg-sub { font-size: 12px; color: var(--dsw-alias-label-tertiary, #7A828C); margin-top: -8px; }
.jm-cfg-field { display: flex; flex-direction: column; gap: 6px; }
.jm-cfg-head { display: flex; align-items: baseline; gap: 8px; flex-wrap: wrap; }
.jm-cfg-label { font-size: 12.5px; font-weight: 500; }
.jm-cfg-hint { font-size: 11.5px; color: var(--dsw-alias-label-tertiary, #7A828C); line-height: 1.6; }
.jm-cfg-input { width: 120px; background: var(--dsw-alias-bg-l2, #1D2126); border: 1px solid var(--dsw-alias-border-l2, #31373F); color: var(--dsw-alias-label-primary, #E4E8ED); border-radius: 6px; padding: 5px 9px; font: inherit; }
.jm-cfg-input:focus { outline: 2px solid var(--dsw-alias-state-business-primary, #5B8DB8); outline-offset: 1px; }
.jm-cfg-check { display: flex; align-items: center; gap: 8px; cursor: pointer; }
.jm-cfg-check input { accent-color: #5B8DB8; }
.jm-cfg-textarea { width: 100%; box-sizing: border-box; background: var(--dsw-alias-bg-l2, #1D2126); border: 1px solid var(--dsw-alias-border-l2, #31373F); color: var(--dsw-alias-label-primary, #E4E8ED); border-radius: 6px; padding: 8px 10px; font-family: 'SF Mono', Menlo, Consolas, monospace; font-size: 12px; line-height: 1.6; resize: vertical; }
.jm-cfg-textarea:focus { outline: 2px solid var(--dsw-alias-state-business-primary, #5B8DB8); outline-offset: 1px; }
.jm-cfg-actions { display: flex; align-items: center; gap: 10px; margin-top: 2px; }
.jm-cfg-save { background: var(--dsw-alias-state-business-primary, #5B8DB8); color: #fff; border: none; border-radius: 6px; padding: 6px 18px; font: inherit; cursor: pointer; }
.jm-cfg-save:disabled { opacity: .5; cursor: default; }
.jm-cfg-reset, .jm-cfg-mini { background: none; border: 1px solid var(--dsw-alias-border-l2, #31373F); color: var(--dsw-alias-label-tertiary, #7A828C); border-radius: 6px; padding: 5px 14px; font: inherit; cursor: pointer; }
.jm-cfg-reset:hover, .jm-cfg-mini:hover { color: var(--dsw-alias-label-primary, #E4E8ED); }
.jm-cfg-mini { padding: 3px 10px; font-size: 11.5px; align-self: flex-start; }
.jm-cfg-msg { font-size: 12px; color: var(--dsw-alias-label-tertiary, #7A828C); }
`
    ...
    function renderPanel() {
      ...
      if (state.selected) {
        bodyHtml = '<div class="jm-view jm-split">'
          + '<div class="jm-preview"><div class="jm-phead"><button class="jm-back" data-jm-act="back">‹ 返回列表</button><span class="jm-ptitle">' + esc(selItem ? selItem.title : '') + '</span><div class="jm-right"><div class="jm-seg"><span data-jm-act="seg-preview" class="' + (state.seg === 'preview' ? 'jm-on' : '') + '">◉ 预览</span><span data-jm-act="seg-source" class="' + (state.seg === 'source' ? 'jm-on' : '') + '"></> 源码</span></div></div></div>'
          + (state.docLoading ? '<div class="jm-loading">加载中…</div>' : '<div class="jm-pbody">'
            + (state.seg === 'source'
              ? '<pre class="jm-pre">' + esc(state.doc ? state.doc.text : '') + '</pre>'
              : (selItem ? '<p class="jm-psummary">' + esc(selItem.summary) + '</p><div class="jm-pchips">' + chipsHtml(selItem.sources, selItem.tags[0]) + '</div>' : '') + (state.doc && state.doc.error ? '<div class="jm-empty">' + esc(state.doc.error) + '</div>' : mdHtml(stripFrontmatter(state.doc ? state.doc.text : ''))))
            + '</div>')
          + '</div>'
      ...
      panelRoot.innerHTML = '<div class="jm-topbar">'
        + '<button class="jm-close" data-jm-act="close">× 关闭</button>'
        ...
        + '<span class="jm-count">' + esc(count) + '</span></div>'
    ...
    function ConfigCard() {
        ...
        fetchJson({ action: 'config' })
          .then((r) => {
            if (r && r.ok) {
              ...
              setDraft(normalizeDraft(r.config))
            } else setMsg('加载失败：' + ((r && r.reason) || '未知错误'))
          })
          .catch((e) => setMsg('加载失败：' + String((e && e.message) || e)))
      }
      ...
      function save() {
        ...
        if (!Number.isInteger(payload.maxEntries) || !Number.isInteger(payload.maxPersonas) || !Number.isInteger(payload.maxBytes)) {
          setMsg('条数与字节上限请填整数')
          return
        }
        setBusy(true)
        ...
        fetch('/api/jinji-memory?action=config', {
          ...
          .then((r) => {
            if (r && r.ok) {
              setDraft(normalizeDraft(r.config))
              setMsg('已保存 ✓ 新会话立即生效')
            } else setMsg('保存失败：' + ((r && r.reason) || '未知错误'))
          })
      ...
      function boolRow(labelText, hint, key) {
        return h('div', { className: 'jm-cfg-field', key },
          h('label', { className: 'jm-cfg-check' },
            h('input', { type: 'checkbox', checked: draft[key], disabled: busy, onChange: (e) => setDraft({ ...draft, [key]: e.target.checked }) }),
            h('span', { className: 'jm-cfg-label' }, labelText)),
          h('div', { className: 'jm-cfg-hint' }, hint))
      }
      ...
      function numRow(labelText, hint, key) {
        return h('div', { className: 'jm-cfg-field', key },
          h('div', { className: 'jm-cfg-head' },
            h('span', { className: 'jm-cfg-label' }, labelText),
            h('span', { className: 'jm-cfg-hint' }, hint)),
          h('input', {
      ...
      if (draft === null) {
        return h('div', { className: 'jm-cfg-card' },
          h('div', { className: 'jm-cfg-title' }, '谨迹记忆'),
          h('div', { className: 'jm-cfg-hint' }, msg || '加载中…'))
      }

      return h('div', { className: 'jm-cfg-card' },
        h('div', { className: 'jm-cfg-title' }, '谨迹记忆'),
        h('div', { className: 'jm-cfg-sub' }, 'jinji-memory · 双轨文本记忆（流水日志 + 画像）'),
        ...
        h('div', { className: 'jm-cfg-field', key: 'writeProtocol' },
          h('div', { className: 'jm-cfg-head' },
            ...
            h('span', { className: 'jm-cfg-hint' }, '留空使用内置默认；文本里可用 __MEMORY_ROOT__ 占位记忆根目录')),
          h('textarea', {
            className: 'jm-cfg-textarea', rows: 8, value: draft.writeProtocol, disabled: busy,
            placeholder: '留空使用内置默认规范',
            onChange: (e) => setDraft({ ...draft, writeProtocol: e.target.value }),
          }),
          h('button', {
            type: 'button', className: 'jm-cfg-mini', disabled: busy,
            onClick: () => setDraft({ ...draft, writeProtocol: (meta && meta.protocolBuiltin) || '' }),
          }, '填入内置默认')),
        h('div', { className: 'jm-cfg-actions' },
          h('button', { type: 'button', className: 'jm-cfg-save', disabled: busy, onClick: save }, busy ? '保存中…' : '保存'),
          h('button', { type: 'button', className: 'jm-cfg-reset', disabled: busy, onClick: reload }, '重置'),
          msg ? h('span', { className: 'jm-cfg-msg' }, msg) : null),
        h('div', { className: 'jm-cfg-hint' }, '保存写入记忆根目录下的 ' + ((meta && meta.file) || '.jinji-memory.json') + '，随记忆库一起迁移；root 目录本身在 profile 的 cordis.patch.yml 里配置。'))
    }
