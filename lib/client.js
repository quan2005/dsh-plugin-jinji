...
.jm-searchwrap { flex-shrink: 0; padding: 10px 24px 12px; border-bottom: 1px solid #1c2126; }
.jm-search { width: 100%; box-sizing: border-box; background: #101216; border: 1px solid #23282E; border-radius: 8px; color: #C9CFD6; font: inherit; font-size: 12.5px; padding: 7px 12px; outline: none; transition: border-color .15s; }
.jm-search::placeholder { color: #6b737d; }
.jm-search:focus { border-color: #5B8DB8; }
.jm-more { padding: 20px 0 8px; color: #6b737d; font-size: 12px; text-align: center; }
.jm-empty-search { padding: 64px 24px; }
.jm-card.jm-kb-active { border-color: #5B8DB8; box-shadow: 0 0 0 1px #5B8DB8; }
...
`
    ...
    function renderPanel() {
      ...
      if (searchInput !== null) {
        searchInput.addEventListener('input', () => {
          ...
          state.shown = PAGE_SIZE // 改 query 重置分页
          state.kbIndex = -1
          searchFocusPending = true
          renderPanel()
        })
    ...
    function ConfigCard() {
      ...
      function installPreset() {
        setBusy(true)
        setMsg(null)
        fetch('/api/jinji-memory?action=install-preset', { method: 'POST' })
          .then((r) => r.json())
          .then((r) => {
            if (r && r.ok) {
              setPreset({ id: 'jinji', available: true, installed: true })
              setMsg({ kind: 'ok', text: r.already ? '预设已存在，无需重复安装' : '已安装 ✓ 新建会话时在预设选择器里选「谨迹秘书」即可' })
            } else setMsg({ kind: 'err', text: '安装失败：' + ((r && r.reason) || '未知错误') })
          })
          .catch((e) => setMsg({ kind: 'err', text: '安装失败：' + String((e && e.message) || e) }))
          .finally(() => setBusy(false))
      }
      ...
      if (open) {
        if (draft === null) {
          ...
          body = h('div', { className: 'jm-cfg-body' },
            ...
            preset !== null && preset.available
              ? h('div', { className: 'jm-cfg-field', key: 'preset' },
                  h('div', { className: 'jm-cfg-head' },
                    h('span', { className: 'jm-cfg-label' }, 'Agent 预设「谨迹秘书」'),
                    preset.installed
                      ? h('span', { className: 'jm-cfg-hint' }, '已安装 ✓')
                      : h('button', { type: 'button', className: 'jm-cfg-mini', disabled: busy, onClick: installPreset }, '安装')),
                  h('p', { className: 'jm-cfg-hint' }, '主动书写记忆的能力（何时写、日志/画像格式、建档门槛）由这个预设承载：安装后，新建会话时在预设选择器里选「谨迹秘书」，该会话就会成为记忆管家；普通会话不受影响。预设文件在你的用户预设目录里，可随时在「Agent 预设」页删除。'))
              : null,
            h('div', { className: 'jm-cfg-field' },
