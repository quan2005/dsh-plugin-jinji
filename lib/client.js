/**
 * dsh-plugin-jinji — Client 半：零构建浏览器 bundle。
 *
 * 手写惰性 CJS bundle（学习手册第 8 章格式）：脚本只注册 factory，
 * 浏览器侧 cordis 树 import 'dsh-plugin-jinji' 时物化。
 *
 * 面板与入口均用纯 DOM 渲染（不依赖 React 槽位挂载链路）：
 * - 入口按钮：DOM 注入到 New Session 按钮正下方，getComputedStyle
 *   逐属性拷贝内联样式，主题下与「新会话」像素级一致（幽灵按钮）；
 * - 记忆面板：document.body 全屏挂载，innerHTML 模板 + 安全转义 + 事件绑定；
 * - 数据：fetch('/api/jinji-memory')（Host 半 webServer 路由）。
 */
...
`
    ...
    exports.apply = function (ctx) {
      ctx.effect(() => injectCss(JM_CSS), 'jinji-memory css')
