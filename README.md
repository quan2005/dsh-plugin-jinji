...
- **日志**：每天用录音、截图、粘贴文字记录素材，由 AI 秘书整理成结构化日志，按 `yyMM/DD-标题.md` 存放在 `.journal/memory/`；
- **画像**：长期维护的人物档案（身份、关切、决策模式……）与产品档案，存放在 `.journal/identity/`。
...
适合人群：想让自己的 DeepSeek Harness 开箱即拥有「记忆」的人；想参考「如何在 DeepSeek Harness 里做一个带界面入口的插件」的开发者（每一步设计决策都写成了文档）。
